...
export const _onResourceChange = async function (event) {
  ...
  if (actorData.system[resource].value > actorData.system[resource].max) {
    actorData.system[resource].value = actorData.system[resource].max
  } else if (actorData.system[resource].value < 0) {
    actorData.system[resource].value = 0
  }
...
export const _onDotCounterChange = async function (event) {
  ...
  if (this.actor.system.locked && !parent.has('.hunger-value').length && !parent.has('.bloodpool-value').length) {
    return
  }
  ...
  if (parent.has('.bloodpool-value').length) {
    recalcHunger(actor, index + 1, actor.system.hunger.max)
  }
...
export const _onDotCounterEmpty = async function (event) {
  ...
  if (!(this.actor.type === 'group') && actor.system.locked && !parent.has('.hunger-value').length && !parent.has('.bloodpool-value').length) {
    return
  }
...
function recalcHunger(actorData, value, maxHunger) {
  let currentBloodPool = value
  let maxBloodPool = actorData.system.bloodpool.max

  let newHungerValue = getHunger(currentBloodPool, maxBloodPool, maxHunger)
  actorData.system.hunger.value = newHungerValue
  actorData.update({ 'system.hunger.value': newHungerValue })
}

function getHunger(currentBloodPool, maxBloodPool, maxHunger) {
  if (maxBloodPool === currentBloodPool) {
    return 0;
  }
  if (currentBloodPool === 0) {
    return maxHunger;
  }
  // ex: 10 blood max, 5 max hunger, so step is 10 / 5 = 2
  let step = Math.abs(maxBloodPool) / maxHunger;
  // ex: step is 2, current blood pool is 9, so current hunger is 5 - (integer part of 9 / 2) = 1
  return maxHunger - Math.trunc(currentBloodPool / step)
}
